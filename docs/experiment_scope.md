...
On CPU-only Codespace the upstream `.cuda()/set_device` calls require a minimal device patch or a GPU host (Kaggle P100 via `~/.kaggle/kaggle.json`). Any patch is documented in `docs/reproduction_report.md` § Deviations and does not change noise/architecture/loss.
