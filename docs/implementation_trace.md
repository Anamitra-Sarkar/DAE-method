...
Source revision `507201c` (MIT-licensed implementation, see LICENSE).
Branch: `main` in `Anamitra-Sarkar/DAE-method`.
...
- Codespace has no GPU; the reconstruction code calls `.cuda()` unconditionally. CPU run needs minimal device patch or GPU host (Kaggle P100).
...
- `thop`, `medpy`, `SimpleITK`, `joblib` must be installed; project dependencies to be pinned during env setup.
