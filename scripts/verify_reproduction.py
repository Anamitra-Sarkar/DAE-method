...
need("README.md", "front page")
need("REPRODUCTION_GUIDE.md", "reproduction guide")
need("scripts/inspect_brats.py", "inspect script")
need("scripts/run_dae_reproduction.sh", "rerun entrypoint")

# checkpoint: 119MB exceeds GitHub's 100MB file limit -> release asset + pointer.
# Accept either the file or the pointer with release URL + sha256.
ckpt = ROOT / "artifacts/checkpoints/model.pt"
ptr = ROOT / "artifacts/checkpoints/README.md"
if not ckpt.exists():
    if not ptr.exists() or "releases/tag" not in ptr.read_text(errors="replace"):
        fails.append("missing checkpoint artifacts/checkpoints/model.pt and no release pointer")
need("artifacts/checkpoints/README.md", "checkpoint pointer")
...
for d in ["README.md", "REPRODUCTION_GUIDE.md", "docs/reproduction_report.md", "docs/DAE_methodology.md"]:
    if (ROOT / d).exists():
        no_placeholders(d)
